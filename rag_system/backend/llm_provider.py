...
class GroqProvider:
    ...
    def _try_model(
        ...
        """
        Try generating a response using a specific Groq model.

        Returns:
            Generated text if successful, otherwise None.
        """
        try:
            ...
            logger.exception(f"Groq model {model} failed")
            return None
    ...
    def generate(self, prompt: str, max_tokens: int = 512, temperature: float = 0.7) -> str:
        ...
        raise RuntimeError(
            "Groq API failed for all configured models. "
...
You MUST answer ONLY using the information contained in the supplied context.

Instructions:

1. Never use outside knowledge.
2. If the answer exists in the context, provide a clear and concise explanation.
3. Quote important details whenever possible.
4. If the answer is not present, respond exactly:
